...
const Dashboard = () => {
  ...
  const fetchStats = async () => {
    try {
      ...
      consumptionSnapshot.forEach((doc) => {
        ...
        if (record.paid) {
          totalPaid += Number(record.total);
        } else {
          totalPending += Number(record.total);
        }
      });
  ...
  const handleFilter = async () => {
    ...
    try {
      ...
      // Manually join with customers
      const records = [];
      for (const doc of querySnapshot.docs) {
        const record = doc.data();
        const customerDoc = await getDoc(doc(db, "customers", record.customer_id));
        records.push({
          ...record,
          customerName: customerDoc.exists() ? customerDoc.data().name : "Cliente desconhecido",
        });
      }
      records.sort((a, b) => a.consumption_date.localeCompare(b.consumption_date));
