...
const Payments = () => {
  ...
  const fetchRecords = async () => {
    ...
    try {
      ...
      const recordsData = recordsSnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      })) as Omit<ConsumptionRecord, "customers">[];

      const customerIds = [...new Set(recordsData.map((r) => r.customer_id))];
      const customersData = await Promise.all(
        customerIds.map(async (id) => {
          const docRef = doc(db, "customers", id);
          const docSnap = await getDoc(docRef);
          return { id, ...(docSnap.data() as Omit<Customer, "id">) };
        })
      );
      const customersMap = new Map(customersData.map((c) => [c.id, c]));

      const hydratedRecords = recordsData.map((record) => ({
        ...record,
        customers: customersMap.get(record.customer_id) || { name: "Desconhecido", phone: null },
      }));
  ...
  const handleDelete = async (recordId: string) => {
    try {
      await deleteDoc(doc(db, "consumption_records", recordId));
